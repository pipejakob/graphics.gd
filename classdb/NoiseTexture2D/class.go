...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.NoiseTexture2D{gdclass.NewNoiseTexture2D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetNoiseTexture2D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.NoiseTexture2D{gdclass.NewNoiseTexture2D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("NoiseTexture2D", func(ptr gdreference.Object) any { return Instance{gdclass.NewNoiseTexture2D(ptr)} })
}
