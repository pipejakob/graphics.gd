...
/*
Child [CanvasItem] nodes of a [CanvasGroup] are drawn as a single object. It allows to e.g. draw overlapping translucent 2D nodes without causing the overlapping sections to be more opaque than intended (set the [CanvasItem.SelfModulate] property on the [CanvasGroup] to achieve this effect).
...
func (self *class) SetObject(obj [1]gdreference.Object) bool {
	if gdextension.Host.Objects.Cast(gdreference.GetObject(obj[0]), otype) != 0 {
		self[0] = gdclass.NewCanvasGroup(obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.CanvasGroup{gdclass.NewCanvasGroup(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetCanvasGroup(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.CanvasGroup{gdclass.NewCanvasGroup(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (o class) AsCanvasGroup() Advanced                   { return Advanced(o) }
func (o Instance) AsCanvasGroup() Instance                { return o }
func (o *Extension[T]) AsCanvasGroup() Instance           { return o.Super() }
...
func init() {
	gdclass.Register("CanvasGroup", func(ptr gdreference.Object) any { return Instance{gdclass.NewCanvasGroup(ptr)} })
}
