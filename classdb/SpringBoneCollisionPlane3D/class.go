...
func init() {
	gd.Links = append(gd.Links, func() {
		sname = gdextension.Host.Strings.Intern.UTF8("SpringBoneCollisionPlane3D")
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.SpringBoneCollisionPlane3D{gdclass.NewSpringBoneCollisionPlane3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetSpringBoneCollisionPlane3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.SpringBoneCollisionPlane3D{gdclass.NewSpringBoneCollisionPlane3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
	gd.ObjectNotification(casted.AsObject()[0], 0, false)
	return casted
}

func (o class) AsSpringBoneCollisionPlane3D() Advanced         { return Advanced(o) }
func (o Instance) AsSpringBoneCollisionPlane3D() Instance      { return o }
func (o *Extension[T]) AsSpringBoneCollisionPlane3D() Instance { return o.Super() }
...
func init() {
	gdclass.Register("SpringBoneCollisionPlane3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewSpringBoneCollisionPlane3D(ptr)} })
}
