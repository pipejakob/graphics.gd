...
func (self class) IsPickable() bool { //gd:CollisionObject2D.is_pickable
	var r_ret = jumponly.Call[bool](gd.ObjectChecked(self.AsObject()), methods.is_pickable, gdextension.SizeBool, &struct{}{})
	var ret = r_ret
	return ret
}
...
func init() {
	gdclass.Register("CollisionObject2D", func(ptr gdreference.Object) any { return Instance{gdclass.NewCollisionObject2D(ptr)} })
}

type DisableMode int64 //gd:CollisionObject2D.DisableMode

const (
	// When [Node.ProcessMode] is set to [Node.ProcessModeDisabled], remove from the physics simulation to stop all physics interactions with this [CollisionObject2D].
	//
	...
	// When [Node.ProcessMode] is set to [Node.ProcessModeDisabled], make the body static. Doesn't affect [Area2D]. [PhysicsBody2D] can't be affected by forces or other bodies while static.
	//
