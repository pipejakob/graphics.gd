...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.BoneConstraint3D{gdclass.NewBoneConstraint3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetBoneConstraint3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.BoneConstraint3D{gdclass.NewBoneConstraint3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("BoneConstraint3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewBoneConstraint3D(ptr)} })
}
