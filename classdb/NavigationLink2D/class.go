...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.NavigationLink2D{gdclass.NewNavigationLink2D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetNavigationLink2D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.NavigationLink2D{gdclass.NewNavigationLink2D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (self class) GetEndPosition() Vector2.XY { //gd:NavigationLink2D.get_end_position
	var r_ret = jumponly.Call[Vector2.XY](gd.ObjectChecked(self.AsObject()), methods.get_end_position, gdextension.SizeVector2, &struct{}{})
	runtime.KeepAlive(self[0].Anchor())
	var ret = r_ret
	return ret
}
...
func init() {
	gdclass.Register("NavigationLink2D", func(ptr gdreference.Object) any { return Instance{gdclass.NewNavigationLink2D(ptr)} })
}
