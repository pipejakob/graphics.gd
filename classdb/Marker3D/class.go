...
func init() {
	gd.Links = append(gd.Links, func() {
		sname = gdextension.Host.Strings.Intern.UTF8("Marker3D")
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.Marker3D{gdclass.NewMarker3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetMarker3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.Marker3D{gdclass.NewMarker3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("Marker3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewMarker3D(ptr)} })
}
