...
Note: If [Viewport.UseXr] is true, most of the camera properties are overridden by the active [XRInterface]. The only properties that can be trusted are the near and far planes.
...
func New() Instance {
	...
	casted := Instance([1]gdclass.XRCamera3D{gdclass.NewXRCamera3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (o class) AsCamera3D() Camera3D.Advanced         { return Camera3D.Advanced{gdclass.NewCamera3D(o[0].AsObject()[0])} }
func (o *Extension[T]) AsCamera3D() Camera3D.Instance { return o.Super().AsCamera3D() }
func (o Instance) AsCamera3D() Camera3D.Instance      { return Camera3D.Instance{gdclass.NewCamera3D(o[0].AsObject()[0])} }
