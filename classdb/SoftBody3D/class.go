...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.SoftBody3D{gdclass.NewSoftBody3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetSoftBody3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.SoftBody3D{gdclass.NewSoftBody3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (o class) AsSoftBody3D() Advanced         { return Advanced(o) }
...
func init() {
	gdclass.Register("SoftBody3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewSoftBody3D(ptr)} })
}
