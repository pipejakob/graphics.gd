...
/*
A 2D polyline shape, intended for use in physics. Used internally in [CollisionPolygon2D] when it's in [Collisionpolygon2d.BuildSegments] mode.
...
Performance: Due to its complexity, [ConcavePolygonShape2D] is the slowest 2D collision shape to check collisions against. Its use should generally be limited to level geometry. If the polyline is closed, [CollisionPolygon2D]'s [Collisionpolygon2d.BuildSolids] mode can be used, which decomposes the polygon into convex ones; see [ConvexPolygonShape2D]'s documentation for instructions.
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.ConcavePolygonShape2D{gdclass.NewConcavePolygonShape2D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetConcavePolygonShape2D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.ConcavePolygonShape2D{gdclass.NewConcavePolygonShape2D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("ConcavePolygonShape2D", func(ptr gdreference.Object) any { return Instance{gdclass.NewConcavePolygonShape2D(ptr)} })
}
