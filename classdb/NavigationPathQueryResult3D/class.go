...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.NavigationPathQueryResult3D{gdclass.NewNavigationPathQueryResult3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetNavigationPathQueryResult3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.NavigationPathQueryResult3D{gdclass.NewNavigationPathQueryResult3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (o *Extension[T]) AsNavigationPathQueryResult3D() Instance { return o.Super() }
...
func init() {
	gdclass.Register("NavigationPathQueryResult3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewNavigationPathQueryResult3D(ptr)} })
}

type PathSegmentType int64 //gd:NavigationPathQueryResult3D.PathSegmentType
