...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.AnimationTree{gdclass.NewAnimationTree(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetAnimationTree(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.AnimationTree{gdclass.NewAnimationTree(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (self Instance) OnAnimationPlayerChanged(cb func(), flags ...Signal.Flags) Instance {
	...
	gd.ObjectConnect(self.AsObject()[0], gd.NewStringName("animation_player_changed"), gd.NewCallable(cb), int64(flags_together))
	return self
}
...
func init() {
	gdclass.Register("AnimationTree", func(ptr gdreference.Object) any { return Instance{gdclass.NewAnimationTree(ptr)} })
}
