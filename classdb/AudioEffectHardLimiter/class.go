...
If clipping is desired, consider [Audioeffectdistortion.ModeClip].
*/
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.AudioEffectHardLimiter{gdclass.NewAudioEffectHardLimiter(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetAudioEffectHardLimiter(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.AudioEffectHardLimiter{gdclass.NewAudioEffectHardLimiter(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("AudioEffectHardLimiter", func(ptr gdreference.Object) any { return Instance{gdclass.NewAudioEffectHardLimiter(ptr)} })
}
