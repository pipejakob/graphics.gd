...
/*
[LightmapProbe] represents the position of a single manually placed probe for dynamic object lighting with [LightmapGI]. Lightmap probes affect the lighting of [GeometryInstance3D]-derived nodes that have their [GeometryInstance3D.GiMode] set to [Geometryinstance3d.GiModeDynamic].

Typically, [LightmapGI] probes are placed automatically by setting [LightmapGI.GenerateProbesSubdiv] to a value other than [Lightmapgi.GenerateProbesDisabled]. By creating [LightmapProbe] nodes before baking lightmaps, you can add more probes in specific areas for greater detail, or disable automatic generation and rely only on manually placed probes instead.
...
[LightmapGI.GenerateProbesSubdiv]: https://pkg.go.dev/graphics.gd/classdb/LightmapGI#Instance.GenerateProbesSubdiv
...
func init() {
	gd.Links = append(gd.Links, func() {
		sname = gdextension.Host.Strings.Intern.UTF8("LightmapProbe")
...
func (self *class) SetObject(obj [1]gdreference.Object) bool {
	if gdextension.Host.Objects.Cast(gdreference.GetObject(obj[0]), otype) != 0 {
		self[0] = gdclass.NewLightmapProbe(obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.LightmapProbe{gdclass.NewLightmapProbe(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetLightmapProbe(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.LightmapProbe{gdclass.NewLightmapProbe(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
	gd.ObjectNotification(casted.AsObject()[0], 0, false)
	return casted
}

func (o class) AsLightmapProbe() Advanced         { return Advanced(o) }
func (o Instance) AsLightmapProbe() Instance      { return o }
func (o *Extension[T]) AsLightmapProbe() Instance { return o.Super() }
...
func init() {
	gdclass.Register("LightmapProbe", func(ptr gdreference.Object) any { return Instance{gdclass.NewLightmapProbe(ptr)} })
}
