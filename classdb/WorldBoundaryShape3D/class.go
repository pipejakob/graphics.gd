...
[Plane]: https://pkg.go.dev/graphics.gd/classdb/WorldBoundaryShape3D#Instance.Plane
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.WorldBoundaryShape3D{gdclass.NewWorldBoundaryShape3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetWorldBoundaryShape3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.WorldBoundaryShape3D{gdclass.NewWorldBoundaryShape3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("WorldBoundaryShape3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewWorldBoundaryShape3D(ptr)} })
}
