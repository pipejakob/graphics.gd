...
[ResourcePreloader]: https://pkg.go.dev/graphics.gd/classdb/ResourcePreloader
*/
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.ResourcePreloader{gdclass.NewResourcePreloader(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetResourcePreloader(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.ResourcePreloader{gdclass.NewResourcePreloader(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("ResourcePreloader", func(ptr gdreference.Object) any { return Instance{gdclass.NewResourcePreloader(ptr)} })
}
