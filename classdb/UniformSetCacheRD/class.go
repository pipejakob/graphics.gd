...
func GetCache(shader RID.Shader, set int, uniforms []RDUniform.Instance) RID.UniformSet { //gd:UniformSetCacheRD.get_cache
	self := Instance{}
	return RID.UniformSet(RID.UniformSet(Advanced(self).GetCache(RID.Any(shader), int64(set), gd.ArrayFromSlice[Array.Contains[[1]gdclass.RDUniform]](uniforms))))
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.UniformSetCacheRD{gdclass.NewUniformSetCacheRD(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetUniformSetCacheRD(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.UniformSetCacheRD{gdclass.NewUniformSetCacheRD(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("UniformSetCacheRD", func(ptr gdreference.Object) any { return Instance{gdclass.NewUniformSetCacheRD(ptr)} })
}
