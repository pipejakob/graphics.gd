...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.Mutex{gdclass.NewMutex(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetMutex(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.Mutex{gdclass.NewMutex(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (o *Extension[T]) AsMutex() Instance   { return o.Super() }
...
func init() {
	gdclass.Register("Mutex", func(ptr gdreference.Object) any { return Instance{gdclass.NewMutex(ptr)} })
}
