...
[CapsuleShape2D]: https://pkg.go.dev/graphics.gd/classdb/CapsuleShape2D
...
func init() {
	gd.Links = append(gd.Links, func() {
		sname = gdextension.Host.Strings.Intern.UTF8("CircleShape2D")
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.CircleShape2D{gdclass.NewCircleShape2D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetCircleShape2D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.CircleShape2D{gdclass.NewCircleShape2D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
The circle's radius.
*/
func (self Instance) Radius() Float.X { //gd:CircleShape2D.radius
	return Float.X(Float.X(class(self).GetRadius()))
}

// SetRadius sets the property returned by [GetRadius]. Returns the instance, so that property settings can be chained.
func (self Instance) SetRadius(value Float.X) Instance { //gd:CircleShape2D.radius
...
func init() {
	gdclass.Register("CircleShape2D", func(ptr gdreference.Object) any { return Instance{gdclass.NewCircleShape2D(ptr)} })
}
