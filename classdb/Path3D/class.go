...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.Path3D{gdclass.NewPath3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetPath3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.Path3D{gdclass.NewPath3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (self Instance) OnCurveChanged(cb func(), flags ...Signal.Flags) Instance {
	...
	gd.ObjectConnect(self.AsObject()[0], gd.NewStringName("curve_changed"), gd.NewCallable(cb), int64(flags_together))
	return self
}

func (self class) CurveChanged() Signal.Any {
	return Signal.Via(gd.WrapSignal(gd.NewSignalOf(self.AsObject(), gd.NewStringName(`curve_changed`))))
}
...
func (self Instance) OnDebugColorChanged(cb func(), flags ...Signal.Flags) Instance {
	...
	gd.ObjectConnect(self.AsObject()[0], gd.NewStringName("debug_color_changed"), gd.NewCallable(cb), int64(flags_together))
	return self
}

func (self class) DebugColorChanged() Signal.Any {
	return Signal.Via(gd.WrapSignal(gd.NewSignalOf(self.AsObject(), gd.NewStringName(`debug_color_changed`))))
}
...
func init() {
	gdclass.Register("Path3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewPath3D(ptr)} })
}
