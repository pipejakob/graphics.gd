...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.PhysicsDirectSpaceState2D{gdclass.NewPhysicsDirectSpaceState2D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetPhysicsDirectSpaceState2D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.PhysicsDirectSpaceState2D{gdclass.NewPhysicsDirectSpaceState2D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("PhysicsDirectSpaceState2D", func(ptr gdreference.Object) any { return Instance{gdclass.NewPhysicsDirectSpaceState2D(ptr)} })
}
