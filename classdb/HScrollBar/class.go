...
func init() {
	gd.Links = append(gd.Links, func() {
		sname = gdextension.Host.Strings.Intern.UTF8("HScrollBar")
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.HScrollBar{gdclass.NewHScrollBar(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetHScrollBar(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.HScrollBar{gdclass.NewHScrollBar(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("HScrollBar", func(ptr gdreference.Object) any { return Instance{gdclass.NewHScrollBar(ptr)} })
}
