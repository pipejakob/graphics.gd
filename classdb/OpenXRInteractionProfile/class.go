...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.OpenXRInteractionProfile{gdclass.NewOpenXRInteractionProfile(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetOpenXRInteractionProfile(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.OpenXRInteractionProfile{gdclass.NewOpenXRInteractionProfile(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("OpenXRInteractionProfile", func(ptr gdreference.Object) any { return Instance{gdclass.NewOpenXRInteractionProfile(ptr)} })
}
