...
[Texture3D.GetData]: https://pkg.go.dev/graphics.gd/classdb/Texture3D#Instance.GetData
...
func (self *class) SetObject(obj [1]gdreference.Object) bool {
	if gdextension.Host.Objects.Cast(gdreference.GetObject(obj[0]), otype) != 0 {
		self[0] = gdclass.NewNoiseTexture3D(obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.NoiseTexture3D{gdclass.NewNoiseTexture3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetNoiseTexture3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.NoiseTexture3D{gdclass.NewNoiseTexture3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
[Depth]: https://pkg.go.dev/graphics.gd/classdb/NoiseTexture3D#Instance.Depth
[FastNoiseLite]: https://pkg.go.dev/graphics.gd/classdb/FastNoiseLite
[Height]: https://pkg.go.dev/graphics.gd/classdb/NoiseTexture3D#Instance.Height
[Noise]: https://pkg.go.dev/graphics.gd/classdb/Noise
...
func (o class) AsNoiseTexture3D() Advanced              { return Advanced(o) }
func (o Instance) AsNoiseTexture3D() Instance           { return o }
func (o *Extension[T]) AsNoiseTexture3D() Instance      { return o.Super() }
...
func init() {
	gdclass.Register("NoiseTexture3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewNoiseTexture3D(ptr)} })
}
