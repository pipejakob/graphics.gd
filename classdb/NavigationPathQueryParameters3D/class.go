...
func New() Instance {
	if !gd.Linked {
		...
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				...
				gdreference.SetObject(gdclass.GetNavigationPathQueryParameters3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.NavigationPathQueryParameters3D{gdclass.NewNavigationPathQueryParameters3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
/*
The list of region [Resource.ID]s that will be excluded from the path query. Use [NavigationRegion3D.GetRid] to get the [Resource.ID] associated with a [NavigationRegion3D] node.
...
/*
The list of region [Resource.ID]s that will be included by the path query. Use [NavigationRegion3D.GetRid] to get the [Resource.ID] associated with a [NavigationRegion3D] node. If left empty all regions are included. If a region ends up being both included and excluded at the same time it will be excluded.
...
func (o class) AsNavigationPathQueryParameters3D() Advanced         { return Advanced(o) }
func (o Instance) AsNavigationPathQueryParameters3D() Instance      { return o }
func (o *Extension[T]) AsNavigationPathQueryParameters3D() Instance { return o.Super() }
