...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.ScrollBar{gdclass.NewScrollBar(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetScrollBar(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.ScrollBar{gdclass.NewScrollBar(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (self Instance) OnScrolling(cb func(), flags ...Signal.Flags) Instance {
	...
	gd.ObjectConnect(self.AsObject()[0], gd.NewStringName("scrolling"), gd.NewCallable(cb), int64(flags_together))
	return self
}

func (self class) Scrolling() Signal.Any {
	return Signal.Via(gd.WrapSignal(gd.NewSignalOf(self.AsObject(), gd.NewStringName(`scrolling`))))
}
...
func init() {
	gdclass.Register("ScrollBar", func(ptr gdreference.Object) any { return Instance{gdclass.NewScrollBar(ptr)} })
}
