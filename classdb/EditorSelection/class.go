...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.EditorSelection{gdclass.NewEditorSelection(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetEditorSelection(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.EditorSelection{gdclass.NewEditorSelection(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (self class) RemoveNode(node [1]gdclass.Node) { //gd:EditorSelection.remove_node
	noescape.Call[struct{}](gd.ObjectChecked(self.AsObject()), methods.remove_node, 0|(gdextension.SizeObject<<4), &struct{ node gdextension.Object }{gdextension.Object(gdreference.GetObject(gdclass.GetNode(node[0])[0]))})
}
...
func init() {
	gdclass.Register("EditorSelection", func(ptr gdreference.Object) any { return Instance{gdclass.NewEditorSelection(ptr)} })
}
