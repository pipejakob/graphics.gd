...
[ProcessAction]: https://pkg.go.dev/graphics.gd/classdb/JSONRPC#Instance.ProcessAction
*/
func (self MoreArgs) ProcessAction(action any, recurse bool) any { //gd:JSONRPC.process_action
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.JSONRPC{gdclass.NewJSONRPC(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetJSONRPC(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.JSONRPC{gdclass.NewJSONRPC(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("JSONRPC", func(ptr gdreference.Object) any { return Instance{gdclass.NewJSONRPC(ptr)} })
}
