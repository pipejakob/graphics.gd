...
func LoadFromFile(path string) AudioStreamOggVorbis.Instance { //gd:ResourceImporterOggVorbis.load_from_file
	self := Instance{}
	return AudioStreamOggVorbis.Instance(Advanced(self).LoadFromFile(String.From(path)))
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.ResourceImporterOggVorbis{gdclass.NewResourceImporterOggVorbis(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetResourceImporterOggVorbis(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.ResourceImporterOggVorbis{gdclass.NewResourceImporterOggVorbis(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (o class) AsResourceImporterOggVorbis() Advanced         { return Advanced(o) }
...
func init() {
	gdclass.Register("ResourceImporterOggVorbis", func(ptr gdreference.Object) any { return Instance{gdclass.NewResourceImporterOggVorbis(ptr)} })
}
