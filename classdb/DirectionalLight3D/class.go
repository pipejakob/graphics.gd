...
func (self *class) SetObject(obj [1]gdreference.Object) bool {
	if gdextension.Host.Objects.Cast(gdreference.GetObject(obj[0]), otype) != 0 {
		self[0] = gdclass.NewDirectionalLight3D(obj[0])
		return true
	}
	return false
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.DirectionalLight3D{gdclass.NewDirectionalLight3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetDirectionalLight3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.DirectionalLight3D{gdclass.NewDirectionalLight3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (o class) AsDirectionalLight3D() Advanced         { return Advanced(o) }
func (o Instance) AsDirectionalLight3D() Instance      { return o }
func (o *Extension[T]) AsDirectionalLight3D() Instance { return o.Super() }
...
func init() {
	gdclass.Register("DirectionalLight3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewDirectionalLight3D(ptr)} })
}
