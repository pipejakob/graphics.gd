...
func init() {
	gd.Links = append(gd.Links, func() {
		sname = gdextension.Host.Strings.Intern.UTF8("JNISingleton")
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.JNISingleton{gdclass.NewJNISingleton(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetJNISingleton(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.JNISingleton{gdclass.NewJNISingleton(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("JNISingleton", func(ptr gdreference.Object) any { return Instance{gdclass.NewJNISingleton(ptr)} })
}
