...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.ExternalTexture{gdclass.NewExternalTexture(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetExternalTexture(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.ExternalTexture{gdclass.NewExternalTexture(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func init() {
	gdclass.Register("ExternalTexture", func(ptr gdreference.Object) any { return Instance{gdclass.NewExternalTexture(ptr)} })
}
