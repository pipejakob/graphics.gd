...
type Instance [1]gdclass.GDScriptSyntaxHighlighter
...
func init() {
	gd.Links = append(gd.Links, func() {
		sname = gdextension.Host.Strings.Intern.UTF8("GDScriptSyntaxHighlighter")
...
type Any interface {
	gd.IsClass
	AsGDScriptSyntaxHighlighter() Instance
}
...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.GDScriptSyntaxHighlighter{gdclass.NewGDScriptSyntaxHighlighter(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetGDScriptSyntaxHighlighter(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.GDScriptSyntaxHighlighter{gdclass.NewGDScriptSyntaxHighlighter(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (self class) Virtual(name string) reflect.Value {
	switch name {
	default:
		return gd.VirtualByName(EditorSyntaxHighlighter.Advanced(self.AsEditorSyntaxHighlighter()), name)
	}
}
...
func (self Instance) Virtual(name string) reflect.Value {
	switch name {
	default:
		return gd.VirtualByName(EditorSyntaxHighlighter.Instance(self.AsEditorSyntaxHighlighter()), name)
	}
}
func init() {
	gdclass.Register("GDScriptSyntaxHighlighter", func(ptr gdreference.Object) any { return Instance{gdclass.NewGDScriptSyntaxHighlighter(ptr)} })
}
