...
func New() Instance {
	if !gd.Linked {
		var placeholder = Instance([1]gdclass.SpringArm3D{gdclass.NewSpringArm3D(gdreference.NewObject())})
		gd.StartupFunctions = append(gd.StartupFunctions, func() {
			if gd.Linked {
				raw, _ := gdreference.EndObject(New().AsObject()[0])
				gdreference.SetObject(gdclass.GetSpringArm3D(placeholder[0])[0], raw)
				gd.RegisterCleanup(func() {
	...
	casted := Instance([1]gdclass.SpringArm3D{gdclass.NewSpringArm3D(gdreference.OwnObject(gdextension.Host.Objects.Make(sname), gd.Free))})
...
func (self class) GetHitLength() float64 { //gd:SpringArm3D.get_hit_length
	var r_ret = jumponly.Call[float64](gd.ObjectChecked(self.AsObject()), methods.get_hit_length, gdextension.SizeFloat, &struct{}{})
	var ret = r_ret
	return ret
}
...
func (o class) AsSpringArm3D() Advanced           { return Advanced(o) }
func (o Instance) AsSpringArm3D() Instance        { return o }
func (o *Extension[T]) AsSpringArm3D() Instance   { return o.Super() }
...
func init() {
	gdclass.Register("SpringArm3D", func(ptr gdreference.Object) any { return Instance{gdclass.NewSpringArm3D(ptr)} })
}
