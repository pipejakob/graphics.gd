...
func (classDB ClassDB) generateObjectPackage(class gdjson.Class, singleton bool, global_enums map[string]gdjson.Enum) error {
	...
	defer file.Close()
	generateFileHeader(file, classDB, class)
	if !class.IsEphemeral {
		generateImports(file, class, singleton)
	}
	fmt.Fprintln(file)
	var local_enums = make(map[string]bool)
	if !class.IsEphemeral {
		generateTypeDecls(file, classDB, class, singleton, hasVirtual)
		getter_setters := propertyAccessors(class)
		generateSimpleMethods(classDB, file, class, singleton, getter_setters)
		generateRelocatedMethods(classDB, file, class)
		generateAdvanced(file, class, singleton)
		if !singleton {
		...
		generateCasts(file, classDB, class, singleton)
		generateVirtuals(file, classDB, class)
		...
		fmt.Fprintf(file, "}\n")
		generateClassEnums(file, classDB, class)
	}
	generateRenumeratedEnums(file, classDB, class, global_enums, local_enums)
	generateStructables(file)
	generateConstants(file, class)
	return nil
}

// generateFileHeader emits the DO-NOT-EDIT banner, the optional package doc
// comment, and the package clause.
func generateFileHeader(file io.Writer, classDB ClassDB, class gdjson.Class) {
...
// generateImports emits the fixed import set, the per-class imports computed by
// gdtype.ImportsForClass, and the blank-identifier assertions that keep those
// imports referenced regardless of which APIs the class actually uses.
func generateImports(file io.Writer, class gdjson.Class, singleton bool) {
	if singleton {
...
// generateTypeDecls emits the package-level type surface: the typed ID, the
// Extension/Singleton embedding helpers, the Instance type, the cached method
// table with its init/cleanup, and either the singleton accessor or the
// MoreArgs/Nil/Any/Interface declarations for ordinary classes.
func generateTypeDecls(file io.Writer, classDB ClassDB, class gdjson.Class, singleton, hasVirtual bool) {
...
// propertyAccessors returns the set of method names that back a property's
// getter or setter; these are skipped by the simple-method generator since
// they are exposed as fields instead.
func propertyAccessors(class gdjson.Class) map[string]bool {
	getter_setters := make(map[string]bool)
	...
	return getter_setters
}

// generateSimpleMethods emits the convenience (simple) API for each method
// that is neither relocated nor a property accessor, plus a second overload
// when the method has applicable default arguments.
func generateSimpleMethods(classDB ClassDB, file io.Writer, class gdjson.Class, singleton bool, getter_setters map[string]bool) {
	for _, method := range class.Methods {
...
// generateRelocatedMethods emits the simple API for methods relocated onto
// this class from a peer class (per gdjson.RelocationsReverse): the peer
// becomes the receiver/first argument and self-typed arguments are remapped.
func generateRelocatedMethods(classDB ClassDB, file io.Writer, class gdjson.Class) {
...
// generateAdvanced emits the low-level Advanced/class type, its AsObject
// accessor, and the SetObject implementations for class, Instance and the
// Extension wrapper.
func generateAdvanced(file io.Writer, class gdjson.Class, singleton bool) {
...
// generateCasts emits the As<Class> conversions: an identity cast to this
// class, then a chain of casts up the inheritance tree (skipping singletons),
// with a special case for RefCounted.
func generateCasts(file io.Writer, classDB ClassDB, class gdjson.Class, singleton bool) {
	if class.Inherits == "" {
		return
	}
	if !singleton {
...
// generateVirtuals emits the Virtual(name) dispatch method for both the
// advanced (class) and Instance receivers, falling through to the parent
// class's virtual table when the name is not one of this class's virtuals.
func generateVirtuals(file io.Writer, classDB ClassDB, class gdjson.Class) {
	super := classDB[class.Inherits]
...
// generateClassEnums emits the class's own (non-renumerated) enums. Only
// called for non-ephemeral classes; RenderingDevice's enums are emitted
// against the Rendering package instead (see generateRenumeratedEnums).
func generateClassEnums(file io.Writer, classDB ClassDB, class gdjson.Class) {
...
// generateRenumeratedEnums emits enums relocated into this package: the
// RenderingDevice enums for the synthetic Rendering package, any enums
// renumerated here via gdjson.Renumeration, and any local global-enum
// aliases. Runs for every class (including the ephemeral Rendering/GUI).
func generateRenumeratedEnums(file io.Writer, classDB ClassDB, class gdjson.Class, global_enums map[string]gdjson.Enum, local_enums map[string]bool) {
...
// generateConstants emits the class constants, grouped by the prefix-to-type
// mapping in gdjson.Consitution: a typed alias per distinct retype, then each
// constant typed by the first prefix it matches.
func generateConstants(file io.Writer, class gdjson.Class) {
